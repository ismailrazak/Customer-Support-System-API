...
class ChatConsumer(WebsocketConsumer):
        ...
        if hasattr(self.user, "customer_profile"):
         if self.user.customer_profile == self.conversation.customer:
            ...
            if not self.conversation.customer_rep:
                ...
                free_customer_rep = CustomerSupportRepProfile.objects.filter(max_capacity__gt=0).first()
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        Message.objects.create(text=text_data_json['message'], user=self.user, conversation=self.conversation)
